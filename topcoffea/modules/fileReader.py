...
def GetListOfWCs(fname):
  ...
  if 'WCnames' not in tree.keys():
    ...
    for idx,i in enumerate(wc_info):
      ...
      #   - The leading "-" indicates the given fragement is the trailing end of the previous WC name
      #   - The following logic is supposed to put those fragemntes back together into the WC name
      if not wc_fragment.startswith("-"):
          wc_names_lst.append(wc_fragment)
      else:
          leftover = wc_fragment[1:]                    # This leftover part of the WC goes with the prvious one (but get rid of leading "-")
